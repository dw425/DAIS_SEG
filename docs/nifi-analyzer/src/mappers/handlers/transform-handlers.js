...
export function handleTransformProcessor(p, props, varName, inputVar, existingCode, existingConf, translateNELtoPySpark) {
  ...
  if (p.type === 'ValidateCsv') {
    code = `# Validate CSV: ${p.name}\nfrom pyspark.sql.functions import col\ndf_${varName} = df_${inputVar}\n_corrupt = df_${varName}.filter(col("_corrupt_record").isNotNull()) if "_corrupt_record" in df_${varName}.columns else spark.createDataFrame([], df_${varName}.schema)\nprint(f"[VALIDATE] CSV validation: {_corrupt.count()} corrupt records found")`;
    conf = 0.93;
    return { code, conf };
  }
