"""Informatica to Databricks mapper."""
...
_YAML_PATH = Path(__file__).parent.parent.parent / "constants" / "processor_maps" / "informatica_databricks.yaml"
        ...
        if entry:
            ...
            mappings.append(
                MappingEntry(
                    ...
                    mapped=False,
                    confidence=0.0,
                    code=f"# UNMAPPED: Informatica {p.type}",
                    notes="Manual migration required",
                )
            )

    return AssessmentResult(mappings=mappings, packages=[], unmapped_count=unmapped)
